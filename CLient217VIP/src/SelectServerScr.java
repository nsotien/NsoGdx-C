public final class SelectServerScr extends mScreen implements IActionListener {
   ...
   public static int selServerIdx = -1;

   public SelectServerScr() {
      if (selServerIdx == -1) {
         selServerIdx = RMS.gameAC("idxServer");
         if (selServerIdx == -1)
            selServerIdx = 0;
      }

      if (UpdateServer.listIP != null && selServerIdx >= 0 && selServerIdx < UpdateServer.listIP.length) {
         GameMidlet.IP = UpdateServer.listIP[selServerIdx];
         GameMidlet.PORT = UpdateServer.listPort[selServerIdx];
         GameMidlet.serverLogin = UpdateServer.serverLoginList[selServerIdx];
      }
   }

   public void updateMenuLabels() {
      // Refresh username from RMS to ensure synchronization
      uname = RMS.loadRMSString("acc");
      if (uname == null)
         uname = "";

      String serverName = "Unknown";
      if (UpdateServer.listName != null && selServerIdx >= 0 && selServerIdx < UpdateServer.listName.length) {
         serverName = UpdateServer.listName[selServerIdx];
      }

      String mayChuLabel = (mResources.mayChu != null) ? mResources.mayChu : "Máy Chủ: ";

      if (uname.equals("") && unameChange.equals("")) {
         menu = new String[] { mResources.choiMoi, mResources.doiTaiKhoan, mayChuLabel + serverName,
               mResources.gameXK };
      } else {
         menu = new String[] { mResources.choiTiep, mResources.choiMoi, mResources.doiTaiKhoan,
               mayChuLabel + serverName,
               mResources.gameXK };
      }
   }
   ...
   public final void update() {
      ...
      if (cmdChoiMoi == null) {
         ...
         cmd = new Command[][] {
               { cmdChoiMoi, cmdDoiTaiKhoan, cmdChonServer, cmdUpdLinkSv },
               { cmdChoiTiep, cmdChoiMoi, cmdDoiTaiKhoan, cmdChonServer, cmdUpdLinkSv }
         };
      }
   ...
   public final void paint(mGraphics var1) {
      ...
      var1.gameAA(LoginScr.imgTitle, GameCanvas.hw - LoginScr.imgTitle.getWidth() / 2,
            this.popupY + 10 - LoginScr.imgTitle.getHeight() / 2, 0);
      ...
      for (int var3 = 0; var3 < menu.length; var3++) {
         ...
         String label = menu[var3];
         if (!uname.equals("") && var3 == 0) {
            label += (!unameChange.equals("") ? ": " + unameChange : (uname.startsWith("tmpusr") ? "" : ": " + uname));
         }
         mFont.tahoma_7b_white.gameAA(var1, label, this.popupX + this.popupW / 2, var2 + var3 * 35 + 8, 2);
      }
   ...
   public final void setOffset() {
      ...
      if (GameCanvas.isPointerJustRelease
            && GameCanvas.gameAB(this.popupX + 10, this.popupY + 35, this.popupW - 10, 175)) {
      ...
      switch (var1) {
         case 1000:
            if (gameAG() && !uname.equals("")) {
               GameCanvas.gameAA(
                     mResources.gameXB, new Command(mResources.choiTiep, this, 10001, null),
         ...
         case 1001:
            if (gameAG() && !uname.equals("") && unameChange.equals("")) {
               GameCanvas.gameAA(
                     mResources.gameXB, new Command(mResources.gameXG, this, 10004, null),
                     new Command(mResources.gameCU, GameCanvas.instance, 8882, null));
               return;
            }
         ...
         case 1002:
            ...
            for (int i1 = 0; i1 < UpdateServer.serverST.length; i1++) {
               var5.addElement(new Command(UpdateServer.listName[i1], this, i1 + 20000, null));
            }

            GameCanvas.menu.gameAA(var5);
            GameCanvas.menu.menuSelectedItem = selServerIdx;
         ...
         default:
            if (var1 >= 20000 && var1 < 20000 + UpdateServer.serverST.length) {
               selServerIdx = var1 - 20000;
               GameCanvas.menu.showMenu = false;
               GameMidlet.IP = UpdateServer.listIP[selServerIdx];
               GameMidlet.PORT = UpdateServer.listPort[selServerIdx];
               GameMidlet.serverLogin = UpdateServer.serverLoginList[selServerIdx];
               RMS.gameAA("idxServer", selServerIdx);
               RMS.gameAA("indServer", UpdateServer.serverST[selServerIdx]);

               // Force update the menu array immediately
               updateMenuLabels();

               // Show visual confirmation on screen
               //GameCanvas.setText("Đã chọn máy chủ: " + UpdateServer.listName[selServerIdx]);
            }
