...
public final class TileMap {
   ...
   public static void a(int var0, InputStream var1) {
      try {
         ...
         int payloadSize = var1.available();
         be[var0] = new byte[payloadSize];
         var1.read(be[var0]);
         if (DebugLog.DEBUG) ;
      } catch (Exception var3) {
         if (DebugLog.DEBUG) ;
   ...
   public static void h() {
      ...
      try {
         ...
         if (DebugLog.DEBUG) {
            ...
            sb.append("[EDO-HIST] mapID=").append((int)mapID).append(" nonZero=").append(nonZero).append(" maxCode=").append(maxCode).append(" codes={");
            boolean first = true;
            for (int c = 0; c < 256; c++) {
               if (freq[c] > 0) {
                  if (!first) sb.append(',');
                  sb.append(c).append(':').append(freq[c]);
                  first = false;
               }
            }
            sb.append("}");
   ...
   public static void i() {
      // P2 A1#8: g8 cross-check khi load tile assets — fail thi set mapID=-1 (render loi tile mismatch).
