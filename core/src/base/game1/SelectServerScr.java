...
import base.game1.screen.ScreenQLTK;
...
public final class SelectServerScr extends mScreen implements IActionListener {
   ...
   public static void f() {
      // Tìm server theo tên đã lưu (không phụ thuộc thứ tự worker trả về)
      ...
      // Chỉ set server, KHÔNG ghi đè serverName trong RMS
      applyServer(found);
   }

   // Apply server không lưu tên (dùng khi load).
   // P3D N2 lazy: neu m[x] empty -> fetch detail truoc khi assign.
   private static void applyServer(int var0) {
      ...
      // P3D: lazy fetch — block call. Neu fail (mat mang), giu nguyen IP truoc.
   ...
   public final void a() {
      // Tự động set typeClient nếu chưa có (tránh hiện popup khi đổi tab)
      if (RMS.c("typeClient") < 0) {
         RMS.a("typeClient", com.monkey.nso.NSO.enabledClientTypes[0]);
      }
      ...
      // Clamp: không để panel đè command bar đáy (ab - 26)
      ...
      super.a = new Command1("Phiên bản", GameCanvas.instance, 10792, (Object)null);
      ...
      // Đọc server đã lưu từ RMS (tìm theo tên, fallback index)
      f();
   ...
   public final void d() {
      // Giữ super.b = QLTK, lưu menu command vào v
      if (h.equals("") && j.equals("")) {
   ...
   public final void a(int var1, Object var2) {
      if (var1 >= 20000) {
         ...
         // P3D N2 lazy: ensure server detail loaded truoc khi assign
               ...
               ScreenManager.showOnGL(new Runnable() {
                  public void run() {
                     if (ScreenManager.screenCustomServer == null) {
                        ScreenManager.screenCustomServer = new base.game1.screen.ScreenCustomServer();
                     }
