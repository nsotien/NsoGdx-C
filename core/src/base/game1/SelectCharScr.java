...
public final class SelectCharScr extends mScreen implements IActionListener {
   ...
   private void g() {
      DebugLog.trace("SELECT_CHAR", "g() called h=" + this.h + " name=" + (this.n != null && this.h >= 0 && this.h < this.n.length ? this.n[this.h] : "?"));
      DebugLog.dumpLicense();
      if (this.n[this.h] != null) {
         ig_login = this.n[this.h];
         DebugLog.trace("SELECT_CHAR", "Sending char select packet for: " + this.n[this.h]);
         Service.gI().b(this.n[this.h]);
         DebugLog.trace("SELECT_CHAR", "Packet sent, setting GameCanvas.f=true (loading screen ON)");
         GameCanvas.b(TextGame.y);
         GameCanvas.f = true;
      } else {
         DebugLog.trace("SELECT_CHAR", "No char at slot " + this.h + " → go to CreateChar");
         CreateCharScr.f().a();
      }
