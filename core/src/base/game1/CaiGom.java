...
public class CaiGom {
   ...
   // State resume cho acc gom — khi AutoGom bị addmode(AutoLayDo) thay thế,
   // AutoLayDo set mode=null lúc xong → scheduler nhìn runningState>=0 tạo lại AutoGom.
   ...
   // Lưu mode gốc (AutoTuDanh/AutoTansat/AutoDanhVong/...) trước khi scheduler kick AutoGom
   // để khi gom xong tự restore → user không bị đứng im sau gom (bám Class_fk.Class_ad bên 148_lau).
   public static volatile Auto runningModeTemp = null;
   // Track trade result để phát hiện fail (items không giảm trong bag sau trade).
   // Fail 2 lần liên tiếp → dừng gom (đối phương có thể đầy bag hoặc huỷ trade).
