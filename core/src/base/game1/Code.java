...
public class Code implements Runnable {
   ...
   public static int muaNpcTemplateID = -1;    // NPC template ID trên map (để tìm + đi tới NPC)
   public static int muaNpcID = -1;            // Type cửa hàng (param cho Service.a buy)
  ...
  public static boolean isZoneNpc(Npc npc) {
   ...
   private static void gkn_openSetting() {
      ScreenManager.showOnGL(new Runnable() { public void run() {
         ScreenManager.show(new base.game1.screen.ScreenSettingGKN());
      } });
   }
   ...
   public void start() {
      if (!isStart) {
         ...
         isStart = true;
         base.game1.screen.ScreenSettingSkill.loadSettings();
         base.game1.screen.SetupNVHN.loadSettings();
...
//         GameScr.gI().a("ADMIN", (String)NewScr.listText.elementAt(1), false);
   ...
   public void run() {
      while(isStart) {
         try {
            ...
            // P2 A1#3: g3 cross-check moi 50 iteration — fail thi auto tu dung silent (mode=null).
            ...
            // P2 N6 D2: shouldDegrade -> 10% chance auto tu dung (khach phai bat lai).
            ...
            // --- Scheduler: Auto gom ---
            // Resume AutoGom khi sub-auto (AutoLayDo/AutoGiaoDich) xong -> mode=null nhưng state chưa DONE
            ...
            // Time-triggered start (pattern giống AutoDanhVong scheduler)
            if (CaiGom.enabled && CaiGom.gioNhan >= 0 && CaiGom.phutNhan >= 0
               ...
               if (curDayG != CaiGom.gomCC
                  ...
                  if (DebugLog.DEBUG) ;
                  // Lưu mode gốc (AutoTansat/AutoDanhVong/...) để sau khi gom xong restore lại (bám 148_lau Class_fd.Class_ai)
            ...
            if (mode != null) {
               ...
               try { tusat_hetmp(); } catch (Throwable t) { ; ; }
               try { dunghpmp();    } catch (Throwable t) { ; ; }
               try { tinhluyen();   } catch (Throwable t) { ; ; }
               ...
               // Port 148_lau Class_fd:1036 — Auto cất rương theo Item Cất của CaiGom.
               // Điều kiện: autoCat bật + rương đã load + có ID cần cất + không trong lúc trade.
               // Chạy mỗi tick main loop (global feature, giống 148_lau).
               // Port TGAME: skip khi đang restore snapshot trang bị cuối phiên DV
               if (!holdFilterSale
                     && !wasPkMode
                     && CaiGom.autoCat
                     ...
                     for (int bi = 0; bi < bagX.length; bi++) {
                        ...
                        for (int ci = 0; ci < catIdsX.length; ci++) {
                           if (itX.template.id == catIdsX[ci]) {
                              GameScr.pickNPC(5, 0, 0);      // đi tới NPC rương
                              Service.gI().e(itX.indexUI);   // cất bag → box
                              sentX++;
                              break;
                           }
                        }
                     }
                     if (DebugLog.DEBUG && sentX > 0) ;
                  }
               }
   ...
   public static boolean lenhchat(String var0) {
      if (DebugLog.DEBUG) ;
      try {
         return lenhchatInner(var0);
      } catch (Throwable t) {
         DebugLog.error("[CHAT] Exception in lenhchat('" + var0 + "')", t);
         try { popup_chat("Loi chat: " + t.getClass().getSimpleName() + " - " + t.getMessage()); } catch (Exception ignore) {}
         return false;
      }
   }

   private static boolean lenhchatInner(String var0) {
      int var2 = 0;
      ...
      if (var34.equals("layall")) {
         ...
         ScreenManager.showOnGL(new Runnable() { public void run() { ScreenManager.show(new base.game1.screen.MenuHangDong()); } });
         return true;
   ...
   popup_chat("Bật auto HD");
         ...
         ScreenManager.showOnGL(new Runnable() { public void run() { ScreenManager.show(new base.game1.screen.Setting()); } });
         return true;
      } else if (var34.equals("ky")) {
         ScreenManager.showOnGL(new Runnable() { public void run() { ScreenManager.show(new base.game1.screen.KichYen()); } });
         return true;
         ...
         // Port Class_fd 148_lau .nhan: check map/zone từ CaiGom, sai map popup (user tự đi),
         // sai zone → bật AutoGomNhan luôn, class tự chuyển zone qua remap().
         ...
         ScreenManager.showOnGL(new Runnable() { public void run() { ScreenManager.show(new base.game1.screen.SetupDanhVong()); } });
         return true;
      } else if (var34.equals("cu")) {
         // mở form Cài up
         ScreenManager.showOnGL(new Runnable() { public void run() { ScreenManager.show(new base.game1.screen.FormCaiUp()); } });
         return true;
      } else if (var34.equals("gomcf") || var34.equals("gomsetting") || var34.equals("atg")) {
         // mở setting Auto Gom
         if (DebugLog.DEBUG) ;
         ScreenManager.showOnGL(new Runnable() { public void run() { ScreenManager.show(new base.game1.screen.ScreenCaiGom()); } });
         return true;
      } else if (var34.equals("gom")) {
         // Manual force-start BÊN NICK CHÍNH (đi gom) — bỏ qua check giờ, test nhanh.
         // Lưu mode cũ để restore khi gom xong.
         ...
         // Nếu đang chạy AutoGom/AutoGomNhan → stop hẳn (restore mode gốc nếu có). Ngược lại toggle scheduler.
         if (mode instanceof AutoGom || mode instanceof AutoGomNhan) {
            ...
            mode = restore;  // null hoặc AutoTansat/... cũ
         ...
         if (!advCA) {
            RMS.a("saveItem", -1);
            popup_chat("Tat auto cat do");
         } else {
            RMS.a("saveItem", 1);
            popup_chat("Bật auto cat do");
         }
         return true;
         ...
         kcts = var2;
         return true;
      } else if (var34.equals("slp")) {
         // slp N — đặt timeout sleep màn (N phút). Ví dụ: slp 1, slp 5
         if (var2 >= 1) {
            com.monkey.nso.SleepManager.timeoutMinutes = var2;
            popup_chat("Sleep sau " + var2 + " phut");
         } else {
            popup_chat("Sleep: " + com.monkey.nso.SleepManager.timeoutMinutes + " phut (slp N de doi)");
         }
         return true;
         ...
         ScreenManager.showOnGL(new Runnable() { public void run() { ScreenManager.show(new base.game1.screen.menu_pkam()); } });
         return true;
         ...
         base.game1.screen.ScreenManager.showOnGL(new Runnable() {
            public void run() { base.game1.screen.ScreenManager.show(new base.game1.screen.SetupVxmm()); }
         });
         return true;
      } else if (var34.equals("atn")) {
         base.game1.screen.ScreenManager.showOnGL(new Runnable() {
            public void run() { base.game1.screen.ScreenManager.show(base.game1.screen.ScreenAutoNhom.gI()); }
         });
         return true;
                     ...
                     if (var34.equals("nst")) {
                        ...
                        if (var34.equals("tsx")) {
                           ...
                           if (var34.equals("die")) {
                              ...
                              ScreenManager.showOnGL(new Runnable() { public void run() {
                                 ScreenManager.show(new base.game1.screen.SetupNVHN());
                              } });
                              return true;
                           } else if (var34.equals("time")) {
                              ScreenManager.showOnGL(new Runnable() { public void run() {
                                 ScreenManager.show(new base.game1.screen.ScreenAutoOnOff());
                              } });
                              return true;
   ...
   public static void loadAutoPt() {
      try {
         ...
         // default (fresh install): autoPtOn=true, autoPtRole=0 đã set ở field
         if (DebugLog.DEBUG) ;
      } catch (Exception e) {
         ;
      }
   ...
   // Tick-based auto luyện ngọc — xử lý 1 target/tick, throttle 700ms
   // Chạy ngoài block (mode != null) → hoạt động cả khi không auto farm
   private static void luyen_ngoc_tick() {
      if (luyenngoc.size() == 0) return;

      // Throttle: 700ms giữa 2 lần merge (giống sleep 700ms của NSOZ)
      if (System.currentTimeMillis() - time_luyen_ngoc < 700L) return;

      // Guard disconnect/relogin
      if (Char.getMyChar() == null || Char.getMyChar().arrItemBag == null) return;

      // Guard UI race — không cướp màn hình user đang thao tác
      ...
      for (int i = 0; i < luyenngoc.size(); ++i) {
         ...
         // B1: target lên max cấp (>=10) → remove
         ...
         // B2: target không còn đúng slot trong bag (bị bán/vứt/đổi map) → remove
         ...
         // Đếm rác cấp 1 (id 652-655, upgrade==1, !isLock, khác target)
         int countRac = 0;
         ...
         // Hợp lệ → mở UI + gom nguyên liệu + gửi packet
         GameScr.gI().doOpenUI(46);
         GameScr.cj = target;   // cj = itemSplit
         GameScr.cb = new Item[24];   // cb = arrItemSplit
         ...
         return;   // xong 1 target/tick, tick sau làm target kế tiếp
      }
   }
