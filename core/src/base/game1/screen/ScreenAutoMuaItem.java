...
public class ScreenAutoMuaItem implements ScreenChild {
    ...
    public ScreenAutoMuaItem() {
        ...
        build();
        if (DebugLog.DEBUG) DebugLog.log("UI", "ScreenAutoMuaItem created");
    }
    ...
    private void save() {
        ...
        if (DebugLog.DEBUG) DebugLog.log("UI", "ScreenAutoMuaItem saved " + s);
    }

    @Override public void resize(int w, int h) {
        stage.getViewport().update(w, h, true);
        UIKit.layoutPanel(stage, panel);
        UIKit.logResize("ScreenAutoMuaItem", stage, w, h);
    }
