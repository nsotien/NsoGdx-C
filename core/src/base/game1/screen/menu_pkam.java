...
import base.game1.AutoChoPK;
import base.game1.AutoDanhPK;
import base.game1.AutoMapKhu;
...
public class menu_pkam implements ScreenChild {
    ...
    public menu_pkam() {
        ...
        build();
        if (DebugLog.DEBUG) DebugLog.log("UI", "menu_pkam created stage=" + stage.getViewport().getWorldWidth()
                + "x" + stage.getViewport().getWorldHeight() + " orientation=" + (UIKit.isPortrait() ? "portrait" : "landscape"));
    }
    ...
    private void build() {
        ...
        saveBtn.addListener(new ChangeListener() {
            @Override public void changed(ChangeEvent e, Actor a) {
                if (DebugLog.DEBUG) DebugLog.log("UI", "menu_pkam saveBtn clicked");
                saveDataPk();
                hide();
            }
        });
        exitBtn.addListener(new ChangeListener() {
            @Override public void changed(ChangeEvent e, Actor a) {
                if (DebugLog.DEBUG) DebugLog.log("UI", "menu_pkam exitBtn clicked");
                hide();
            }
        });
    ...
    private void buildFieldRows(Table p) {
        UIKit.addRow(p, "Khu chờ pk",        tfKhuChoPk  = UIKit.tfDigits(String.valueOf(base.game1.menu_pkam.khucpk)));
        UIKit.addRow(p, "Khu đánh pk",       tfKhuDanhPk = UIKit.tfDigits(String.valueOf(base.game1.menu_pkam.khudpk)));
        UIKit.addRow(p, "ID map PK",         tfIdMapPk   = UIKit.tfDigits(String.valueOf(base.game1.menu_pkam.map)));
        UIKit.addRow(p, "Tọa độ X",          tfToaDoX    = UIKit.tfDigits(String.valueOf(base.game1.menu_pkam.x)));
        UIKit.addRow(p, "Tọa độ Y",          tfToaDoY    = UIKit.tfDigits(String.valueOf(base.game1.menu_pkam.y)));
        UIKit.addRow(p, "Bao nhiêu % thì pk", tfPtPk     = UIKit.tfDigits(String.valueOf(base.game1.menu_pkam.ptpk)));
    }
    ...
    public void saveDataPk() {
        ...
        try {
            ...
            base.game1.menu_pkam.khucpk = Integer.parseInt(this.tfKhuChoPk.getText());
            base.game1.menu_pkam.khudpk = Integer.parseInt(this.tfKhuDanhPk.getText());
            base.game1.menu_pkam.map   = Integer.parseInt(this.tfIdMapPk.getText());
            base.game1.menu_pkam.x     = Integer.parseInt(this.tfToaDoX.getText());
            base.game1.menu_pkam.y     = Integer.parseInt(this.tfToaDoY.getText());
            base.game1.menu_pkam.ptpk  = Long.parseLong(this.tfPtPk.getText());
            AutoMapKhu.showUpInfo = this.cbShowInfo.isChecked();
            RMS.a("isShow", AutoMapKhu.showUpInfo ? 1 : -1);
            var2.writeInt(base.game1.menu_pkam.khucpk);
            var2.writeInt(base.game1.menu_pkam.khudpk);
            var2.writeInt(base.game1.menu_pkam.map);
            var2.writeInt(base.game1.menu_pkam.x);
            var2.writeInt(base.game1.menu_pkam.y);
            var2.writeLong(base.game1.menu_pkam.ptpk);
            ...
            // Re-check sau toggle trên vì mode có thể đã đổi
            ...
            if (DebugLog.DEBUG) DebugLog.log("UI", "menu_pkam saved OK khuCho=" + base.game1.menu_pkam.khucpk
                    + " khuDanh=" + base.game1.menu_pkam.khudpk + " map=" + base.game1.menu_pkam.map
                    + " x=" + base.game1.menu_pkam.x + " y=" + base.game1.menu_pkam.y
                    + " ptpk=" + base.game1.menu_pkam.ptpk + " isShow=" + AutoMapKhu.showUpInfo
                    + " pkam=" + Code.pkam);
        } catch (Exception ex) {
            if (DebugLog.DEBUG) DebugLog.error("menu_pkam saveDataPk failed: " + ex.getMessage(), ex);
        }
    }
    ...
    @Override public void resize(int width, int height) {
        ...
        UIKit.layoutPanel(stage, panel);
        UIKit.logResize("menu_pkam", stage, width, height);
    }

    @Override public void show() {
        isVisible = true;
        UIKit.mountStage(stage);
        if (DebugLog.DEBUG) DebugLog.log("UI", "menu_pkam show");
    }

    @Override public void hide() {
        isVisible = false;
        UIKit.restoreInput();
        if (DebugLog.DEBUG) DebugLog.log("UI", "menu_pkam hide");
    }
