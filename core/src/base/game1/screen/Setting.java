...
public class Setting implements ScreenChild {
    ...
    public Setting() {
        ...
        build();
        if (DebugLog.DEBUG) DebugLog.log("UI", "Setting created");
    }
    ...
    private void build() {
        ...
        if (base.game1.Setting.mode_4 == 1) attackHuman.setChecked(true); else attackAll.setChecked(true);
        ...
        switch (base.game1.Setting.mode_1) {
        ...
        if (base.game1.Setting.mode_2 == 1) danhTrongTam.setChecked(true); else dichChuyen.setChecked(true);
        ...
        if (base.game1.Setting.mode_3 == 1) danhTrongListPK.setChecked(true); else danhTatCa.setChecked(true);
        ...
        content.row();
        hoiSinhLuong = UIKit.cb("Hồi sinh lượng",      base.game1.Setting.hsl);
        hoanLuong    = UIKit.cb("Hoàn lương chi thảo", base.game1.Setting.hlct);
        content.add(hoiSinhLuong).colspan(2).left().row();
        content.add(hoanLuong).colspan(2).left().padBottom(6f).row();

        // Hiếu chiến field
        tfHieuChien = UIKit.tfDigits(String.valueOf(base.game1.Setting.hieuchien));
    ...
    public void saveSetting() {
        ...
        try {
            base.game1.Setting.mode_4 = attackHuman.isChecked() ? 1 : 0;
            if (doSat.isChecked())            base.game1.Setting.mode_1 = 1;
            else if (humanDosat.isChecked())  base.game1.Setting.mode_1 = 2;
            else if (alwayOnDosat.isChecked()) base.game1.Setting.mode_1 = 3;
            else                               base.game1.Setting.mode_1 = 0;
            base.game1.Setting.mode_2    = danhTrongTam.isChecked() ? 1 : 0;
            base.game1.Setting.mode_3    = danhTrongListPK.isChecked() ? 1 : 0;
            base.game1.Setting.hsl       = hoiSinhLuong.isChecked();
            base.game1.Setting.hlct      = hoanLuong.isChecked();
            base.game1.Setting.hieuchien = Integer.parseInt(tfHieuChien.getText());

            var2.writeInt(base.game1.Setting.mode_1);
            var2.writeInt(base.game1.Setting.mode_2);
            var2.writeInt(base.game1.Setting.mode_3);
            var2.writeInt(base.game1.Setting.hieuchien);
            var2.writeBoolean(base.game1.Setting.hsl);
            var2.writeBoolean(base.game1.Setting.hlct);
            var2.writeInt(base.game1.Setting.mode_4);
            RMS.a("SettingPK", ((ByteArrayOutputStream) var1).toByteArray());
            var2.flush();
            var1.flush();
            Code.popup_chat("Lưu cài đặt thành công");
            if (DebugLog.DEBUG) DebugLog.log("UI", "Setting saved mode1=" + base.game1.Setting.mode_1
                    + " mode2=" + base.game1.Setting.mode_2 + " mode3=" + base.game1.Setting.mode_3
                    + " mode4=" + base.game1.Setting.mode_4 + " hsl=" + base.game1.Setting.hsl
                    + " hlct=" + base.game1.Setting.hlct + " hieuchien=" + base.game1.Setting.hieuchien);
        } catch (Exception ex) {
            if (DebugLog.DEBUG) DebugLog.error("Setting save failed: " + ex.getMessage(), ex);
        }
    }
    ...
    @Override public void resize(int w, int h) {
        ...
        UIKit.layoutPanel(stage, panel);
        UIKit.logResize("Setting", stage, w, h);
    }
