...
public class ListBlock implements ScreenChild {
    ...
    public static ListBlock abc;
    public static ListBlock gI() { if (abc == null) abc = new ListBlock(); return abc; }

    public ListBlock() {
        stage = UIKit.newStage();
        loadDataBlock();
        build();
        if (DebugLog.DEBUG) DebugLog.log("UI", "ListBlock created size=" + k.size());
    }

    private void build() {
        panel = UIKit.panel("Danh sách chặn lời mời");
    ...
    private void doAdd() {
        ...
        tfUser.setText("");
        rebuildList();
        if (DebugLog.DEBUG) DebugLog.log("UI", "ListBlock add '" + name + "' size=" + k.size());
    }
            ...
            UIKit.addListRow(listTable, name, new Runnable() {
                @Override public void run() {
                    if (index >= 0 && index < k.size()) {
                        ...
                        saveDataBlock();
                        rebuildList();
                        if (DebugLog.DEBUG) DebugLog.log("UI", "ListBlock remove idx=" + index + " size=" + k.size());
                    }
                }
            });
        }
    ...
    public void saveDataBlock() {
        try {
            ...
            RMS.a("blockList", baos.toByteArray());
            baos.close();
            dos.close();
        } catch (Exception e) {
            if (DebugLog.DEBUG) DebugLog.error("ListBlock save failed: " + e.getMessage(), e);
        }
    }

    private void loadDataBlock() {
        try {
            byte[] data = RMS.a("blockList");
            if (data != null) {
            ...
            if (DebugLog.DEBUG) DebugLog.error("ListBlock load failed: " + ex.getMessage(), ex);
        }
    }

    /** Kiểm tra nick có trong block list không. Giữ nguyên API cũ (Controller gọi). */
    ...
    @Override public void resize(int w, int h) {
        ...
        UIKit.layoutPanel(stage, panel);
        UIKit.logResize("ListBlock", stage, w, h);
    }
