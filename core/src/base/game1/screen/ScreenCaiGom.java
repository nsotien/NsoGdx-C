...
public class ScreenCaiGom implements ScreenChild {
    ...
    public ScreenCaiGom() {
        ...
        build();
        if (DebugLog.DEBUG) DebugLog.log("UI", "ScreenCaiGom created");
    }
    ...
    private void saveConfig() {
        try {
            ...
            if (DebugLog.DEBUG) DebugLog.log("UI", "ScreenCaiGom saved enabled=" + CaiGom.enabled
                    + " rcv='" + CaiGom.receiverName + "' time=" + CaiGom.gioNhan + "h" + CaiGom.phutNhan
                    + " map=" + CaiGom.mapNhan + " zone=" + CaiGom.zoneNhan + " autoCat=" + CaiGom.autoCat);
        } catch (Exception e) {
            Code.popup_chat("Lỗi: kiểm tra lại cài đặt!");
            if (DebugLog.DEBUG) DebugLog.error("ScreenCaiGom save failed: " + e.getMessage(), e);
        }
    }
    ...
    @Override public void resize(int w, int h) {
        ...
        UIKit.layoutPanel(stage, panel);
        UIKit.logResize("ScreenCaiGom", stage, w, h);
    }
