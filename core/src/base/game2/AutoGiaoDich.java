...
public class AutoGiaoDich extends Auto {
   ...
   private void reportGomTrade(boolean ok, int sentCount) {
   ...
   private void stopGomTradeFail(int sentCount, boolean closeTrade) {
      reportGomTrade(false, sentCount);
      ...
      if (check_dead(Char.getMyChar())) {
         ...
         if (GameScr.cs) {
            ...
            while(System.currentTimeMillis() - ll < 300L) {
               phong.sleep(100L);
               if (!GameScr.cs) {
                  if (gomTrade) stopGomTradeFail(sentCount, false);
                  return;
               }
            }
            ...
            if (gomTrade) {
               ...
               if (GameScr.cs) {
                  stopGomTradeFail(sentCount, true);
                  return;
               }
               for(int var0 = 0; var0 < 12; ++var0) {
                  if (class_byArr[var0] != null) {
