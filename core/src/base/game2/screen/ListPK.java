...
public final class ListPK implements ScreenChild {
    ...
    private void doAdd() {
        ...
        for (int i = 0; i < base.game2.ListPK.k.size(); i++) {
            if (((String) base.game2.ListPK.k.elementAt(i)).equalsIgnoreCase(name)) {
                tfUser.setText("");
                return;
            }
        }
        base.game2.ListPK.k.addElement(name);
        base.game2.ListPK.saveList();
        tfUser.setText("");
        rebuildList();
    }
    ...
    private void rebuildList() {
        ...
        for (int i = 0; i < base.game2.ListPK.k.size(); i++) {
            final int index = i;
            String name = (String) base.game2.ListPK.k.elementAt(i);
            UIKit.addListRow(listTable, name, new Runnable() {
                @Override public void run() {
                    if (index >= 0 && index < base.game2.ListPK.k.size()) {
                        base.game2.ListPK.k.removeElementAt(index);
                        base.game2.ListPK.saveList();
                        rebuildList();
                    }
                }
