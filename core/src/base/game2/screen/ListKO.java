...
public final class ListKO implements ScreenChild {
    ...
    private void doAdd() {
        ...
        for (int i = 0; i < base.game2.ListKO.k.size(); i++) {
            if (((String) base.game2.ListKO.k.elementAt(i)).equalsIgnoreCase(name)) {
                tfUser.setText("");
                return;
            }
        }
        base.game2.ListKO.k.addElement(name);
        base.game2.ListKO.saveList();
        tfUser.setText("");
        rebuildList();
    }
    ...
    private void rebuildList() {
        ...
        for (int i = 0; i < base.game2.ListKO.k.size(); i++) {
            final int index = i;
            String name = (String) base.game2.ListKO.k.elementAt(i);
            UIKit.addListRow(listTable, name, new Runnable() {
                @Override public void run() {
                    if (index >= 0 && index < base.game2.ListKO.k.size()) {
                        base.game2.ListKO.k.removeElementAt(index);
                        base.game2.ListKO.saveList();
                        rebuildList();
                    }
                }
